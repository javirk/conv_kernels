...
// Exp 54: Add #pragma unroll 4 to load loops
...
static constexpr int TM54 = 128;
static constexpr int TN54 = 128;
static constexpr int BK54 = 64;
static constexpr int PAD_A54 = 8;
static constexpr int PAD_B54 = 8;
static constexpr int NWARPS54 = 8;

__global__ void __launch_bounds__(256, 2)
wmma_unroll4_conv2d_3x3_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    int const block_m = blockIdx.x * TM54;
    int const block_n = blockIdx.y * TN54;

    int const warp_id = threadIdx.x / 32;
    int const warp_row = warp_id / 2;
    int const warp_col = warp_id % 2;
    int const nthreads = NWARPS54 * 32;

    int const my_tm = threadIdx.x % TM54;
    ...
    __shared__ half smem_A[TM54][BK54 + PAD_A54];
    __shared__ half smem_B[BK54][TN54 + PAD_B54];
    ...
    int const total_k_iters = (K + BK54 - 1) / BK54;

    // Precompute: is this thread's spatial position valid?
    int const valid = (my_oh >= 0) ? 1 : 0;

    for (int ki = 0; ki < total_k_iters; ki++) {
        int const k_base = ki * BK54;

        // Load A with partial unroll
        #pragma unroll 4
        for (int idx = threadIdx.x; idx < TM54 * BK54; idx += nthreads) {
            int const tm = idx & (TM54 - 1);  // idx % 128 = idx & 127 (TM is power of 2)
        ...
        #pragma unroll 4
        for (int idx = threadIdx.x; idx < BK54 * TN54; idx += nthreads) {
            int const tk = idx & (BK54 - 1);  // idx % 64 = idx & 63
        ...
        #pragma unroll
        for (int kk = 0; kk < BK54; kk += 16) {
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                wmma::load_matrix_sync(a_frag[mi], &smem_A[wm + mi * 16][kk], BK54 + PAD_A54);
            #pragma unroll
            for (int ni = 0; ni < 4; ni++)
                wmma::load_matrix_sync(b_frag[ni], &smem_B[kk][wn + ni * 16], TN54 + PAD_B54);
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                #pragma unroll
                for (int ni = 0; ni < 4; ni++)
    ...
    __shared__ half smem_store[NWARPS54][16][16];
    int const wm = warp_row * 32;
    int const wn = warp_col * 64;
...
void launch_wmma_unroll4_conv2d_3x3(
    ...
    size_t C_in, size_t C_out, size_t H, size_t W, cudaStream_t stream)
{
    ...
    dim3 const grid(
        (unsigned int)((M + TM54 - 1) / TM54),
        (unsigned int)((C_out + TN54 - 1) / TN54));
    dim3 const block(NWARPS54 * 32);

    wmma_unroll4_conv2d_3x3_kernel<<<grid, block, 0, stream>>>(
