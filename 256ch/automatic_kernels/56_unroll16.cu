...
// Exp 56: Add #pragma unroll 16 to load loops
...
static constexpr int TM56 = 128;
static constexpr int TN56 = 128;
static constexpr int BK56 = 64;
static constexpr int PAD_A56 = 8;
static constexpr int PAD_B56 = 8;
static constexpr int NWARPS56 = 8;

__global__ void __launch_bounds__(256, 2)
wmma_unroll16_conv2d_3x3_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    int const block_m = blockIdx.x * TM56;
    int const block_n = blockIdx.y * TN56;

    int const warp_id = threadIdx.x / 32;
    int const warp_row = warp_id / 2;
    int const warp_col = warp_id % 2;
    int const nthreads = NWARPS56 * 32;

    int const my_tm = threadIdx.x % TM56;
    ...
    __shared__ half smem_A[TM56][BK56 + PAD_A56];
    __shared__ half smem_B[BK56][TN56 + PAD_B56];
    ...
    int const total_k_iters = (K + BK56 - 1) / BK56;

    // Precompute: is this thread's spatial position valid?
    int const valid = (my_oh >= 0) ? 1 : 0;

    for (int ki = 0; ki < total_k_iters; ki++) {
        int const k_base = ki * BK56;

        // Load A with partial unroll
        #pragma unroll 16
        for (int idx = threadIdx.x; idx < TM56 * BK56; idx += nthreads) {
            int const tm = idx & (TM56 - 1);  // idx % 128 = idx & 127 (TM is power of 2)
        ...
        #pragma unroll 16
        for (int idx = threadIdx.x; idx < BK56 * TN56; idx += nthreads) {
            int const tk = idx & (BK56 - 1);  // idx % 64 = idx & 63
        ...
        #pragma unroll
        for (int kk = 0; kk < BK56; kk += 16) {
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                wmma::load_matrix_sync(a_frag[mi], &smem_A[wm + mi * 16][kk], BK56 + PAD_A56);
            #pragma unroll
            for (int ni = 0; ni < 4; ni++)
                wmma::load_matrix_sync(b_frag[ni], &smem_B[kk][wn + ni * 16], TN56 + PAD_B56);
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                #pragma unroll
                for (int ni = 0; ni < 4; ni++)
    ...
    __shared__ half smem_store[NWARPS56][16][16];
    int const wm = warp_row * 32;
    int const wn = warp_col * 64;
...
void launch_wmma_unroll16_conv2d_3x3(
    ...
    size_t C_in, size_t C_out, size_t H, size_t W, cudaStream_t stream)
{
    ...
    dim3 const grid(
        (unsigned int)((M + TM56 - 1) / TM56),
        (unsigned int)((C_out + TN56 - 1) / TN56));
    dim3 const block(NWARPS56 * 32);

    wmma_unroll16_conv2d_3x3_kernel<<<grid, block, 0, stream>>>(
