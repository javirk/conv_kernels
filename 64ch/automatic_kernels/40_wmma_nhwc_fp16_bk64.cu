...
// Experiment 40: FP16 BK=64 — entire C_in in one tile, no ic loop.
//
// Base: exp 39 (FP16 BK=32).
// Change: BK=64 matches C_in exactly. No ic iterations needed.
// Each (fy,fx) is now: load once, compute 4 WMMA k-steps, done.
// No double buffering needed since there's only 1 ic iter.
// SMEM: As[128][72] halves = 18.4KB, Bs[64][72] = 9.2KB ≈ 27.6KB.
// Should fit 3 blocks at 100KB/SM.
...
constexpr int BK40_PAD = BK40 + 8;  // 72
constexpr int BN40_PAD = BN40 + 8;  // 72

__device__ __forceinline__ void load_As_fp16_vec8_40(
...
{
    // As: 128*64 = 8192 halves. float4 = 8 → 1024 loads, 4 per thread
...
__device__ __forceinline__ void load_Bs_fp16_vec8_40(
...
{
    // Bs: 64*64 = 4096 halves. float4 = 8 → 512 loads, 2 per thread
...
__global__ void wmma_nhwc_fp16_bk64_conv2d_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    // No double buffering needed — single ic iteration
    ...
    for (int fy = 0; fy < 3; ++fy)
    {
        ...
        {
            load_As_fp16_vec8_40(As, input, tile_row, tile_col, fy, fx,
                                C_in, H, W, tid, total_threads);
            load_Bs_fp16_vec8_40(Bs, filter_t, block_n, fy, fx,
                                C_in, C_out, tid, total_threads);
            __syncthreads();

            // BK=64, WK=16 → 4 WMMA instructions
            #pragma unroll
    ...
    // Store — As has 128*72 halves = 9216 halves = 18432 bytes = 4608 floats > 4096 needed
...
float profile_nhwc_fp16_bk64_conv2d(size_t C_in, size_t C_out, size_t H, size_t W)
{
    ...
    auto kernel_fn = [&](cudaStream_t s) {
        wmma_nhwc_fp16_bk64_conv2d_kernel<<<grid, block, 0, s>>>(
...
template <typename T>
void launch_wmma_nhwc_fp16_bk64_conv2d_3x3(T* d_output, T const* d_input, T const* d_filter,
                                              ...
                                              cudaStream_t stream)
{
    ...
    dim3 const grid(
        ...
        (out_W + BM40_W - 1) / BM40_W,
        (C_out + BN40 - 1) / BN40);
    wmma_nhwc_fp16_bk64_conv2d_kernel<<<grid, block, 0, stream>>>(
        d_output_hwc, d_input_h, d_filter_t_h, C_in, C_out, H, W);
