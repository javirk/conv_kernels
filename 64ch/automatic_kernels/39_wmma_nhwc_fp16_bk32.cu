...
// Experiment 39: FP16 with BK=32 to halve ic loop iterations.
//
// Base: exp 38 (FP16 global, 128-bit vec loads).
// Change: BK 16→32. With C_in=64: 2 iterations instead of 4.
// Two WMMA K=16 instructions per tile (kk loop: 0,16).
// SMEM: As[2][128][40] halves = 20KB, Bs[2][32][72] halves = 9.2KB ≈ 29KB.
// With 100KB/SM: fits 3 blocks → launch_bounds(256,2) conservative.
...
constexpr int BK39_PAD = BK39 + 8;  // 40
constexpr int BN39_PAD = BN39 + 8;  // 72

__device__ __forceinline__ void load_As_fp16_vec8_39(
...
{
    // As: 128*32 = 4096 halves. float4 = 8 halves → 512 loads, 2 per thread
    constexpr int vec_count = (BM39 * BK39) / 8;  // 512
...
__device__ __forceinline__ void load_Bs_fp16_vec8_39(
...
{
    // Bs: 32*64 = 2048 halves. float4 = 8 halves → 256 loads, 1 per thread
    constexpr int vec_count = (BK39 * BN39) / 8;  // 256
    for (int vi = tid; vi < vec_count; vi += total_threads)
    {
        int const elem = vi * 8;
...
__global__ void wmma_nhwc_fp16_bk32_conv2d_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    for (int fy = 0; fy < 3; ++fy)
    {
        ...
        {
            load_As_fp16_vec8_39(As[0], input, tile_row, tile_col, fy, fx, 0,
                                C_in, H, W, tid, total_threads);
            load_Bs_fp16_vec8_39(Bs[0], filter_t, block_n, fy, fx, 0,
            ...
            for (int ici = 0; ici < num_ic_iters; ++ici)
            {
                ...
                if (ici + 1 < num_ic_iters)
                {
                    ...
                    load_As_fp16_vec8_39(As[next_buf], input, tile_row, tile_col,
                                        fy, fx, next_ic, C_in, H, W, tid, total_threads);
                    load_Bs_fp16_vec8_39(Bs[next_buf], filter_t, block_n, fy, fx,
                                        next_ic, C_in, C_out, tid, total_threads);
                }

                // BK=32, WK=16 → 2 WMMA instructions per tile
                #pragma unroll
...
float profile_nhwc_fp16_bk32_conv2d(size_t C_in, size_t C_out, size_t H, size_t W)
{
    ...
    auto kernel_fn = [&](cudaStream_t s) {
        wmma_nhwc_fp16_bk32_conv2d_kernel<<<grid, block, 0, s>>>(
...
template <typename T>
void launch_wmma_nhwc_fp16_bk32_conv2d_3x3(T* d_output, T const* d_input, T const* d_filter,
                                              ...
                                              cudaStream_t stream)
{
    ...
    dim3 const grid(
        ...
        (out_W + BM39_W - 1) / BM39_W,
        (C_out + BN39 - 1) / BN39);
    wmma_nhwc_fp16_bk32_conv2d_kernel<<<grid, block, 0, stream>>>(
        d_output_hwc, d_input_h, d_filter_t_h, C_in, C_out, H, W);
