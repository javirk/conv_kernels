...
// Load filter weights for multiple output channels into registers.
// Each thread computes multiple output channels (OC_PER_THREAD) at the same spatial location.
// This amortizes the input memory reads across multiple output channels.

template <typename T, int OC_PER_THREAD = 4>
__global__ void filter_reg_conv2d_3x3(T* output, T const* input, T const* filter,
                                       size_t C_in, size_t C_out, size_t H, size_t W)
{
    ...
    {
        // Load the 3x3 input patch once (shared across all output channels)
        T inp[9];
        ...
        // For each output channel, load filter and accumulate
        #pragma unroll
...
template <typename T>
void launch_filter_reg_conv2d_3x3(T* d_output, T const* d_input, T const* d_filter,
                                   size_t C_in, size_t C_out, size_t H, size_t W,
                                   cudaStream_t stream)
{
    ...
    filter_reg_conv2d_3x3<T, OC_PER_THREAD><<<grid, block, 0, stream>>>(
