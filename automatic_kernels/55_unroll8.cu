...
// Exp 55: Add #pragma unroll 8 to load loops
...
static constexpr int TM55 = 128;
static constexpr int TN55 = 128;
static constexpr int BK55 = 64;
static constexpr int PAD_A55 = 8;
static constexpr int PAD_B55 = 8;
static constexpr int NWARPS55 = 8;

__global__ void __launch_bounds__(256, 2)
wmma_unroll8_conv2d_3x3_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    int const block_m = blockIdx.x * TM55;
    int const block_n = blockIdx.y * TN55;

    int const warp_id = threadIdx.x / 32;
    int const warp_row = warp_id / 2;
    int const warp_col = warp_id % 2;
    int const nthreads = NWARPS55 * 32;

    int const my_tm = threadIdx.x % TM55;
    ...
    __shared__ half smem_A[TM55][BK55 + PAD_A55];
    __shared__ half smem_B[BK55][TN55 + PAD_B55];
    ...
    int const total_k_iters = (K + BK55 - 1) / BK55;

    // Precompute: is this thread's spatial position valid?
    int const valid = (my_oh >= 0) ? 1 : 0;

    for (int ki = 0; ki < total_k_iters; ki++) {
        int const k_base = ki * BK55;

        // Load A with partial unroll
        #pragma unroll 8
        for (int idx = threadIdx.x; idx < TM55 * BK55; idx += nthreads) {
            int const tm = idx & (TM55 - 1);  // idx % 128 = idx & 127 (TM is power of 2)
        ...
        #pragma unroll 8
        for (int idx = threadIdx.x; idx < BK55 * TN55; idx += nthreads) {
            int const tk = idx & (BK55 - 1);  // idx % 64 = idx & 63
        ...
        #pragma unroll
        for (int kk = 0; kk < BK55; kk += 16) {
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                wmma::load_matrix_sync(a_frag[mi], &smem_A[wm + mi * 16][kk], BK55 + PAD_A55);
            #pragma unroll
            for (int ni = 0; ni < 4; ni++)
                wmma::load_matrix_sync(b_frag[ni], &smem_B[kk][wn + ni * 16], TN55 + PAD_B55);
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                #pragma unroll
                for (int ni = 0; ni < 4; ni++)
    ...
    __shared__ half smem_store[NWARPS55][16][16];
    int const wm = warp_row * 32;
    int const wn = warp_col * 64;
...
void launch_wmma_unroll8_conv2d_3x3(
    ...
    size_t C_in, size_t C_out, size_t H, size_t W, cudaStream_t stream)
{
    ...
    dim3 const grid(
        (unsigned int)((M + TM55 - 1) / TM55),
        (unsigned int)((C_out + TN55 - 1) / TN55));
    dim3 const block(NWARPS55 * 32);

    wmma_unroll8_conv2d_3x3_kernel<<<grid, block, 0, stream>>>(
