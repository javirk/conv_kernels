...
// Exp 34: Compute k→spatial offsets in registers, not smem
// With int32 arithmetic, div by 9 and 3 are fast (magic multiply+shift).
// Removes k_offsets smem entirely → saves 256 bytes smem + eliminates smem reads.
// Also builds on exp 33's int32 arithmetic.

static constexpr int TM34 = 128;
static constexpr int TN34 = 128;
static constexpr int BK34 = 32;
static constexpr int PAD34 = 8;
static constexpr int NWARPS34 = 8;

__global__ void __launch_bounds__(256, 2)
wmma_reg_offsets_conv2d_3x3_kernel(
    ...
    int C_in, int C_out, int H, int W)
{
    ...
    int const block_m = blockIdx.x * TM34;
    int const block_n = blockIdx.y * TN34;

    int const warp_id = threadIdx.x / 32;
    int const warp_row = warp_id / 2;
    int const warp_col = warp_id % 2;
    int const nthreads = NWARPS34 * 32;

    int const my_tm = threadIdx.x % TM34;
    ...
    __shared__ half smem_A[2][TM34][BK34 + PAD34];
    __shared__ half smem_B[2][BK34][TN34 + PAD34];
    ...
    int const total_k_iters = (K + BK34 - 1) / BK34;

    // Load first A tile — compute offsets in registers
    for (int idx = threadIdx.x; idx < TM34 * BK34; idx += nthreads) {
        int const tm = idx % TM34;
        int const tk = idx / TM34;
    ...
    for (int idx = threadIdx.x; idx < BK34 * TN34; idx += nthreads) {
        int const tk = idx % BK34;
        int const tn = idx / BK34;
    ...
    for (int ki = 0; ki < total_k_iters; ki++) {
        ...
        int const next_k_base = (ki + 1) * BK34;
        ...
        #pragma unroll
        for (int kk = 0; kk < BK34; kk += 16) {
            #pragma unroll
            for (int mi = 0; mi < 2; mi++)
                wmma::load_matrix_sync(a_frag[mi], &smem_A[buf][wm + mi * 16][kk], BK34 + PAD34);
            #pragma unroll
            for (int ni = 0; ni < 4; ni++)
                wmma::load_matrix_sync(b_frag[ni], &smem_B[buf][kk][wn + ni * 16], TN34 + PAD34);
    ...
    // Store output
    __shared__ half smem_store[NWARPS34][16][16];
...
void launch_wmma_reg_offsets_conv2d_3x3(
    ...
    size_t C_in, size_t C_out, size_t H, size_t W, cudaStream_t stream)
{
    ...
    dim3 const grid(
        (unsigned int)((M + TM34 - 1) / TM34),
        (unsigned int)((C_out + TN34 - 1) / TN34));
    dim3 const block(NWARPS34 * 32);

    wmma_reg_offsets_conv2d_3x3_kernel<<<grid, block, 0, stream>>>(
